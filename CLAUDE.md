...
> 📖 **Главная документация проекта:** См. [`README.md`](README.md) для общего обзора системы
> 📋 **Техтребования:** [`docs/REQUIREMENTS.md`](docs/REQUIREMENTS.md)
> 💡 **Концепция:** [`docs/PRODUCT_IDEA.md`](docs/PRODUCT_IDEA.md)
...
## Тестирование HTML файлов

Когда работаешь с HTML/JavaScript:

1. **Открывай файлы через file:// протокол**
   - Используй `mcp__playwright__browser_navigate` с путем типа `file:///полный/путь/к/файлу.html`
   - Пример: `file:///Users/username/project/web/all-cities.html`

2. **Проверяй результат сам**
   - `browser_snapshot` - посмотри DOM структуру
   - `browser_console_messages` - проверь ошибки JavaScript
   - `browser_screenshot` - сделай скриншот страницы

3. **НЕ проси меня проверить вручную** - делай это сам через Playwright

4. **Для локальных файлов не нужен сервер** - используй file:// протокол

### ⚠️ ТЕСТОВЫЙ РЕЖИМ (ОБЯЗАТЕЛЬНО!)

**ВАЖНО:** Всегда используй тестовый режим при автоматическом тестировании страниц с большим количеством данных!

**Как использовать:**
```
http://localhost:8000/all-cities-fixed.html?testMode=true
```

**Зачем это нужно:**
- Загружается только 50 объектов вместо всех 1500+
- Экономия токенов при тестировании (snapshot будет в 30 раз меньше)
- Быстрая загрузка и тестирование основного функционала

**Индикаторы тестового режима:**
- В консоли: `🧪 TEST MODE ENABLED: Limiting to 50 objects`
- На странице: счетчик объектов показывает "(ТЕСТОВЫЙ РЕЖИМ)" оранжевым цветом
- В консоли: `🧪 TEST MODE: Limited to 50 objects`

**Пример использования в тестах:**
```javascript
// ❌ НЕПРАВИЛЬНО - загружает все данные:
await page.goto('http://localhost:8000/all-cities-fixed.html');

// ✅ ПРАВИЛЬНО - тестовый режим:
await page.goto('http://localhost:8000/all-cities-fixed.html?testMode=true');
```

**Когда НЕ использовать тестовый режим:**
- Когда тестируешь работу с полным набором данных
- Когда нужно проверить производительность на реальных данных
- Когда пользователь явно просит протестировать без ограничений
...
│   └── data_example.csv  # Пример структуры данных
...
- Название листа: `Данные` (gid: 1292008891)
...
- Использует прямую ссылку на CSV для получения данных без аутентификации
- **Формат данных:** CSV содержит объекты из **всех городов Свердловской области**
- **Структура группировки:**
  1. Сначала группировка по городам (сортировка по убыванию количества объектов)
  2. Внутри каждого города - группировка по временным периодам
...
- `готово`/`хорошее` → `good`
- `проверяем`/`нормальное` → `normal`
- `плохое` → `bad`
- `руины` → `ruins`
- `не существует` → `not-exists`
- `не доступен` → `not-available`

> 🎨 **Цвета состояний** описаны в [`docs/REQUIREMENTS.md`](docs/REQUIREMENTS.md) (раздел 2.2 и 4.1)
